import { Global, FrameEvent } from '__dirname/global.js';
...
class behavior_Dialogue
{
	...
	displayDialogue(lineIndex)
	{
		...
		if (dialog2)
		{
			...
			for (let i = 0; i < event.length; ++i)
			{
				let evt = new FrameEvent(me, "dialog");
		...
		if (Array.isArray(event[0]))
		{
			for (let i = 0; i < event.length; ++i)
			{
				let evt = new FrameEvent(me, "sync");
				evt.dispatch(g_macroObj, event[i]);
		...
		if (event.length != 0)
		{
			let evt = new FrameEvent(me, "seque");
	...
	onMouseEvent(mouseEvent, mouseWheelDelta)
	{
		...
		if (mouseEvent == 4 && Global.IsSave == false && Global.IsLoad == false)
		{
			...
			document.getElementById('text').style.opacity = document.getElementById('text').style.opacity == "0" ? "1" : "0";
		}
	}
}

export { behavior_Dialogue as default };
